...
while running:
    ...
    mouse_pos = (
        int(window_mouse_pos[0] * DISPLAY_SIZE[0] / WINDOW_SIZE[0]),
        int(window_mouse_pos[1] * DISPLAY_SIZE[1] / WINDOW_SIZE[1]),
    )
    ...
    display.blit(
        school_image, (0, (DISPLAY_SIZE[1] / 2) - (school_image.get_height() / 2))
    )
    ...
    for node in path[1:]:
        node2 = node_map[node]
        draw_smooth_line(
            display, SELECTED_PATH_LINE_COLOR, node1.pos, node2.pos, LINE_WIDTH
        )
        node1 = node2
    ...
    if SHOW_NODES:
        for node in node_map:
            pygame.draw.circle(
                display, NODE_OUTLINE_COLOR, node.pos, NODE_OUTLINE_RADIUS
            )
        ...
        for node in node_map:
            if get_distance(node.pos, mouse_pos) <= NODE_NUM_VISIBLE_DISTANCE:
                ...
                display.blit(
                    node_text,
                    (
                        node.pos[0] - (node_text.get_width() / 2),
                        node.pos[1] - (node_text.get_height() / 2),
                    ),
                )
